...
out/e2e-%: out/minikube-%
	GOOS="$(firstword $(subst -, ,$*))" GOARCH="$(lastword $(subst -, ,$(subst $(IS_EXE), ,$*)))" go test -v -ldflags="${MINIKUBE_LDFLAGS}" -c k8s.io/minikube/test/integration -args --logtostderr=true --tags="$(MINIKUBE_INTEGRATION_BUILD_TAGS)" -o $@
